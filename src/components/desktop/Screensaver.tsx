...
// ---------------------------------------------------------------------------
// Flying Toasters Screensaver — After Dark (1989) recreation
//
// Uses canvas for smooth 60fps animation. Renders:
//   • Toasters with flapping wings (4-frame sprite cycle)
//   • Toast pieces floating alongside
//   • Wispy clouds drifting through
// All fly diagonally from upper-right to lower-left.
// Activates after 3 minutes of idle. Dismissed by any input.
// ---------------------------------------------------------------------------
...
export default function Screensaver() {
  const { idle, dismiss } = useIdleTimer(IDLE_TIMEOUT);
  ...
  // ---- Initialize sprites when going idle ----
  ...
  const animate = useCallback((timestamp: number) => {
    ...
    // Draw toasters with wing animation
  ...
  }, [visible, loaded, initSprites, animate]);
  ...
  return (
    <div
      ...
      style={{
        position: "fixed",
        inset: 0,
        zIndex: 99998, // just below boot sequence
        cursor: "none",
    ...
    >
      ...
      >
        After Dark &bull; Flying Toasters
      </div>
    </div>
  );
}
